...
if size(instance_matrix, 1) ~= N
  fprintf(2, ['the label and instance matrices should have the same number of rows\n']);
  return;
end
...
model = svmtrain(label_matrix, instance_matrix, options);

nclass = model.nr_class;
svmtype = model.Parameters(1);

if nclass ~= 2 || svmtype >= 2
  fprintf(2, ['cannot plot the decision boundary for these SVM problems\n']);
  return
end
...
[Z, acc] = svmpredict(label_test, instance_test, model);
